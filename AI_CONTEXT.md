...
- **AI 設定ファイルの構成**：共通ルールは `AI_CONTEXT.md` に集約する。`CLAUDE.md`・`GEMINI.md` は `@AI_CONTEXT.md` のみ記載。`.github/copilot-instructions.md` は `AI_CONTEXT.md` を参照する旨のみ記載し、個別設定のみ各ファイルに追記する
- **AI 用マークダウンのセクションヘッダ**：日本語ドキュメントでも英語で記載する
