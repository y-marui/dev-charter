...
- **コミット前に `VERSION` を今日の日付（`YYYY-MM-DD`）に更新する**。1日に複数回リリースしない（日付がバージョン識別子のため）。pre-commit フックが自動検証する
  - ローカルの更新コマンド：`UPDATE=1 bash scripts/check-version-date.sh`（`VERSION` と `badge.json` を同時に更新）
  - **クラウド/エージェント環境**：ローカルの pre-commit フックが動作しないため、CI の自動更新ワークフロー（`.github/workflows/update-version.yml`）が `VERSION` を自動的に更新してコミットする。エージェントは VERSION の手動更新を試みるべきだが、漏れた場合は CI が補完する
- **`VERSION` を更新するときは `badge.json` も同期する**：`bash scripts/update-badge.sh` を実行する
