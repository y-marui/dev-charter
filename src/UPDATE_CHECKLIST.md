...
- docs/dev-charter/src/scripts/*.sh（SECURITY_POLICY.md の Setup Steps に従って
  scripts/ へコピー済みの場合）→ 変更されたファイルを再コピーして同期する
  （`cp docs/dev-charter/src/scripts/<ファイル名>.sh scripts/`）
...
- docs/dev-charter/src/scripts/*.sh (if copied into your own scripts/ per
  SECURITY_POLICY.md Setup Steps) → re-copy the changed file(s) to keep your
  scripts/ in sync (`cp docs/dev-charter/src/scripts/<file>.sh scripts/`)
